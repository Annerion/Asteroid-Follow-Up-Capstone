...
def recondense():
	star_map= [[0] * int(width/expansion_factor) for i in range(int(height/expansion_factor))]
	for y in range(0, (height-1)/expansion_factor):
		for x in range(0, (width-1)/expansion_factor):
#			print "recondense: ", x, y
			for yp in range (y*expansion_factor, (y+1)*expansion_factor-1):
				for xp in range (x*expansion_factor, (x+1)*expansion_factor-1):
					if yp<height and xp<width:
						star_map[y][x]+=noise(image.data[yp][xp])
#						star_map[y][x]+=image.data[yp][xp]
	image.data= star_map
#	print star_map
#clear()
makeField()
